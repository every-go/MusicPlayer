...
fun showSongMenu(
    ...
    if (onGoToArtist != null)    popup.menu.add(0, id++, id, "Vai all'artista")
    if (onGoToAlbum != null)     popup.menu.add(0, id++, id, "Vai all'album")
    if (onEdit != null)          popup.menu.add(0, id++, id, "Modifica")
    if (onDelete != null)        popup.menu.add(0, id++, id, "Elimina")

    popup.setOnMenuItemClickListener { item ->
        var currentId = 0
        if (onPlayNext != null && item.itemId == currentId++) {
            onPlayNext()
        ...
        if (onGoToArtist != null && item.itemId == currentId++) {
            val artists = song.artist.split(",").map { it.trim() }.filter { it.isNotEmpty() }
            if (artists.size == 1) {
                onGoToArtist(artists[0])
            } else {
                android.app.AlertDialog.Builder(context)
                    .setTitle("Scegli artista")
                    .setItems(artists.toTypedArray()) { _, which ->
                        onGoToArtist(artists[which])
                    }
                    .show()
            }
            return@setOnMenuItemClickListener true
        }
        if (onGoToAlbum != null && item.itemId == currentId++) {
            onGoToAlbum()
            return@setOnMenuItemClickListener true
        }
        if (onEdit != null && item.itemId == currentId++) {
            onEdit()
            return@setOnMenuItemClickListener true
        }
        if (onDelete != null && item.itemId == currentId) {
            onDelete()
            return@setOnMenuItemClickListener true
        }
        false
