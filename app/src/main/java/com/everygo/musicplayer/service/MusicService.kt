...
class MusicService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        val index = intent?.getIntExtra(PlayerActivity.EXTRA_SONG_INDEX, -1) ?: -1
        if (index >= 0 && queue.playlist.isNotEmpty()) {
            playSong(index)
        }
    ...
    private fun buildPlaybackState(playing: Boolean) =
    ...
    private fun buildNotification(song: Song, art: Bitmap? = null): Notification {
        ...
        val randomIntent = PendingIntent.getService(
            this, 4,
            Intent(this, MusicService::class.java).setAction(ACTION_RANDOM),
            PendingIntent.FLAG_IMMUTABLE
        )
        ...
        return NotificationCompat.Builder(this, CHANNEL_ID)
            ...
            .addAction(
                if (isShuffleEnabled) R.drawable.ic_random_white_24dp
                else R.drawable.ic_random_off_white_24dp,
                "Random",
                randomIntent
            )
            .addAction(R.drawable.ic_skip_previous_white_24dp, "Prev", prevIntent)
            .addAction(
                if (isPlaying) R.drawable.ic_pause_white_24dp else R.drawable.ic_play_white_24dp,
                "Play",
                playPauseIntent
            )
            .addAction(R.drawable.ic_skip_next_white_24dp, "Next", nextIntent)
            .build()
    }

    private fun updateNotification(song: Song, art: Bitmap? = null) {
        val n = buildNotification(song, art)
